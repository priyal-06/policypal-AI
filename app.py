...
# ---------------------- Budget-Based Insurance Suggestions + Premium Calculator ---------------------- #
...
if monthly_budget:
    ...
    if matched:
        for idx, plan in enumerate(matched):
            with st.expander(f"🔹 {plan['name']} ({plan['type']}) – ₹{plan['monthly']}/month"):
                st.markdown(f"📌 **Benefits**: {plan['benefits']}")
                duration = st.number_input(
                    f"Policy Duration (in years) for {plan['name']}",
                    min_value=1, max_value=50, value=5,
                    key=f"duration_{idx}"
                )
                members = st.number_input(
                    f"No. of Members Covered",
                    min_value=1, max_value=10, value=1,
                    key=f"members_{idx}"
                )
                total = plan['monthly'] * 12 * duration * members
                st.success(f"💰 Estimated Total Premium: ₹{total:,}")
    else:
